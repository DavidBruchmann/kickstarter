...
// 				}
	 ...
	 * Fieldname is validated against a TYPO3/MySQL reserved words blacklist.
	 *
	 * @param	string		$str: orginal fieldname
	 * @return	cleaned up fieldname
	 */
	function cleanUpFieldName($str)	{
	require(t3lib_extMgm::extPath('kickstarter').'static/reservedwords.php');
		$fieldName = ereg_replace('[^[:alnum:]_]','',strtolower($str));
		if (!$fieldName || in_array($fieldName,$reservedFields) || in_array($fieldName,$this->usedNames))	{
